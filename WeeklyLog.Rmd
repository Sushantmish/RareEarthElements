...
for (i in 1:15) 
{
  rock_mean <- sapply(rock_row_ranges, function(row) mean(pca_other_eu$x[row, i], na.rm = TRUE))
  par(mar = c(10, 4, 4, 2)) 
  plot(rock_mean, xaxt = "n", col = "red",ylim = range(c(mean_low_lim[,i], mean_high_lim[,i])),
       main = paste("Average PC", i), ylab = paste("Mean PC", i))
  
  arrows(1:58, mean_low_lim[,i], 1:58, mean_high_lim[,i], length=0.03, angle=90, code=3)
  axis(1, at = 1:58, labels = names(rock_mean), las = 2, cex.axis = 0.7)
  grid()
}

for (i in 1:15) 
{
  rock_sd <- sapply(rock_row_ranges, function(row) sd(pca_other_eu$x[row, i], na.rm = TRUE))
  par(mar = c(10, 4, 4, 2)) 
  plot(rock_sd, xaxt = "n", col = "blue",ylim = range(c(sd_low_lim[,i], sd_high_lim[,i])),
       main = paste("Standard Deviation of PC", i), ylab = paste("SD of PC", i))
  
  arrows(1:58, sd_low_lim[,i], 1:58, sd_high_lim[,i], length=0.03, angle=90, code=3)
  axis(1, at = 1:58, labels = names(rock_sd), las = 2, cex.axis = 0.7)
  grid()
}
