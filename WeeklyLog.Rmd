...
```{r}
x_solver <- function(vectors,transformeddata)
{
  A <- as.matrix(vectors)
  B <- as.matrix(transformeddata)
  
  scores <- B %*% A %*% solve(t(A) %*% A)
  return(as.data.frame(scores))
}

solved_scores <- x_solver(combined_vectors,ree_transformed)
colnames(solved_scores) <- c("Total_REE", "Eu_Anomaly", "Linear", "Quad")
#print(solved_scores)
reconstructed_data <- as.matrix(solved_scores) %*% t(combined_vectors)
