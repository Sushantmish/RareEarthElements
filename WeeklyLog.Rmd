...
pca_function <- function(input, use_Other= TRUE,no_EU = TRUE, pc_xaxis="Atomic Number")
{
 filtered_data <- input
 plot_title <- 
 ...
 {
  remove_Eu <- "EU.PPM."
  filtered_data <- input[,!grepl(remove_Eu, names(input)),drop= FALSE]
  plot_title <- " (without Eu)"
 }
 else
 {
  filtered_data <- input
 ...
 if (!use_Other) 
 {
  filtered_data <- filtered_data[, !grepl("Other", names(filtered_data)),drop= FALSE]
  plot_title <- paste0(plot_title, " (without Other)")
 }
 
 #PCA
 pca_result <- prcomp(filtered_data)
 current_names <- row.names(pca_result$rotation)
 clean_names <- sub("\\.[A-Z]+\\.?$", "", current_names)
 row.names(pca_result$rotation) <- clean_names
 
 #Screeplot
 plot(pca_result,main=paste("Screeplot",plot_title))
 
 #Biplots 
 biplot(pca_result,choices = c(1, 2),xlabs=rep('', nrow(filtered_data),main = paste("PC1 vs PC2", plot_title)))
 biplot(pca_result,choices = c(2, 3),xlabs=rep('', nrow(filtered_data),main = paste("PC1 vs PC2", plot_title)))
 
 #PC Plots 
 
 PC_data <- pca_result$rotation 
 
 if (pc_xaxis== "Atomic Number")
 {
  ...
  if (use_Other)
  {
   x_coordinate <- c(x_coordinate, 73)
   ticks <- c(atomic_number, 73)
   label <- c(as.character(atomic_number), "Other")
  } 
  else 
  {
    ticks <- atomic_number
    label <- as.character(atomic_number)
  }
  
 }
 
else if (pc_xaxis == "CN=8 Radius" || pc_xaxis == "CN=6 Radius")
{
 
 if (pc_xaxis == "CN=8 Radius") 
 {
   xcoordinate <- ionic_radius8   # Fixed x_coordinate spelling
   x_label <- "Ionic Radius (CN=8)"
   other_coord <- 0.95 
 } 
 else 
 { 
   xcoordinate <- ionic_radius6   # Fixed x_coordinate spelling
   x_label <- "Ionic Radius (CN=6)"
   other_coord <- 0.85 
 }
 
 
 if (use_Other) # Fixed capitalization: use_Other
 {
   xcoordinate <- c(xcoordinate, other_coord) # Fixed x_coordinate spelling
   ticks <- c(xcoordinate)
   label <- c(as.character(round(xcoordinate[-length(xcoordinate)], 3)), "Other")
 } 
 else 
 {
   ticks <- xcoordinate
   label <- as.character(round(xcoordinate, 3))
 }
 
} 
 
else if (pc_xaxis == "None") 
{
  return(pca_result)
}
 ...
 for (i in 1:5)
 {
  ...
  PC <- PC_data[,pc_number]
  par(las=2)
  plot(xcoordinate,PC, ylab = pc_number,xlab = x_label,main = paste(pc_number, "PC vs.", x_label,  plot_title),xaxt="n")
  axis(side=1,at= ticks,labels= label,cex.axis=0.8)
 }
 return(pca_result)
}
...
```{r}
pca_result_test <- pca_function(pca,use_Other= TRUE,no_EU = TRUE, pc_xaxis="Atomic Number")
