...
vector_1 <- standardize_vectors(v1_initial)
...
v4_initial <- rep(0,n)
seq_for_quadratic <- seq(1,14)
v4_initial[1:14] <- (seq_for_quadratic - mean(seq_for_quadratic))^2
...
colnames(projected_scores) <- c("Total_REE", "Eu_Anomaly", "Linear", "Quad")
...
rock_mean_scores <- aggregate(cbind(Total_REE, Eu_Anomaly, Linear, Quad) ~ Rock_Type,data = projected_scores_filtered,FUN = mean)
```

## Rock scatter Plots

```{r}
axes_pair <- list(c("Total_REE", "Eu_Anomaly"),c("Total_REE", "Linear"),c("Total_REE", "Quad"),c("Eu_Anomaly", "Linear"),c("Eu_Anomaly", "Quad"),c("Linear", "Quad"))
